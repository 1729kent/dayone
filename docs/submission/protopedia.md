...
## 🎯 追加で貼ると勝率が上がる差し込み文（2026-07-07・8割超え狙い）

### 「解決したい課題」欄の末尾に追加（金銭コスト）
```
【課題の金銭コスト】新メンバー1人がオンボーディングで腐ったREADMEに詰まると平均で数時間が溶けます。エンジニア時給5,000円なら1人3時間で約1.5万円、月5人入社する組織で年間約90万円が「気づかないうちに腐ったドキュメント」に支払われている計算です。しかもこの損失は計測されず誰の責任にもなりません。DayOneはこれを腐敗スコアとTime to First Successで可視化します。
```

### 「プロダクトの特徴」欄の末尾に追加（なぜエージェントか＋DevEx＋実証）
```
【なぜLinterでなく自律エージェントか】「READMEの手順が実際に通るか」はその環境で本当に実行してみるまで分かりません（コマンド改名・暗黙の前提・環境差分は文字列検査に現れない）。だからDayOneは実行して失敗を体験し、原因を探索して直すエージェントである必然性があります。低リスクなDocs検証から始めるのは意図的で、サンドボックス・最小権限・許可リストの封じ込めを先に作れば、同じループをマイグレーション実行など高リスク運用へ安全に拡張できます。

【DevExを"測って直す"新指標】開発生産性の可視化が広がる一方、オンボーディングの健全性は誰も測っていません。DayOneの腐敗スコア／Time to First Successは、Four Keysやサイクルタイムが捉えないオンボーディング体験を数値化し、可視化→AI分析→改善(PR)のループで自動的に返す、新しいDevExメトリクスです。

【実証データ】デモの作り話ではありません。実在の人気OSS7プロジェクト（chalk・FastAPI・requests・click・attrs・commander.js・slugify）でクイックスタートを実行し全て腐敗スコア0＝誤検知ゼロを確認。腐敗を注入したフィクスチャでは100%検知し修正PRを自動作成。本番Firestoreには検知→AI修正PR→人間マージ→翌朝スコア0の回復曲線が残り、無人運転とE2E回帰も公開APIで第三者検証できます。デモサイトの「審査員の方はワンクリックでお試しください」から、その場で腐敗検知と誤検知ゼロを体験できます。
```
※7プロジェクト誤検知ゼロは実測確定値（2026-07-07検証）
